    ...
    async roomInitializer(): Promise<void> {
        ...
        if (companionId) {
            this.subscriptions.push(
                this.userService.getCompanionRequest(companionId).subscribe()
            );
        } else {
            this.subscriptions.push(
                this.userService
        ...
        const profileEntries =
            await this.entryService.getEntriesRequestByProfileNotObservable();
        const companionEntries =
            await this.entryService.getEntriesRequestByCompanionNotObservable();
        ...
        if (profileEntries.length === 0 || companionEntries.length === 0) {
            this.subscriptions.push(
                this.roomService.postRoomRequest().subscribe((roomData) => {
                    ...
                    this.subscriptions.push(
                        this.entryService
                            .postEntryRequest(profileEntryValue)
                            .subscribe()
                    );
                    this.subscriptions.push(
                        this.entryService
                            .postEntryRequest(companionEntryValue)
                            .subscribe()
                    );
