...
import { EntryStore } from 'src/app/states/entry/entry.store'
...
export class useEntry {
    ...
    findManyByProfile(): Observable<EntryDto[]> {
        ...
        return this.query.findMany(paramValues).pipe(tap((data) => this.entryStore.updateProfileEntries(data)))
    }

    findManyByCompanion(): Observable<EntryDto[]> {
        const paramValues: string[] = [String(this.userQuery.companionGetter.id)]
        return this.query.findMany(paramValues).pipe(tap((data) => this.entryStore.updateCompanionEntries(data)))
    }
